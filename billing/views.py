...
def create_invoice_view(request, appointment_id=None):
    ...
    if request.method == 'POST':
        ...
        # Check validity for both forms and the formset
        if invoice_form.is_valid() and item_formset.is_valid():
            with transaction.atomic():
                invoice = invoice_form.save() # Save the main invoice first
                
                # Django's formset.save() handles creation, update, and deletion based on has_changed()
                # and the DELETE flag. Our overridden has_changed() will prevent empty extra forms.
                item_formset.instance = invoice # Link formset to the saved invoice
                item_formset.save() # This saves new/changed items and deletes marked ones.

                invoice.calculate_total_amount() # Recalculate after items are saved/deleted
                invoice.save() # Save invoice again to persist total amount

            messages.success(request, f'Invoice {invoice.invoice_number} created successfully!')
            return redirect('billing:invoice_detail', invoice_id=invoice.id)
        else:
            messages.error(request, "Please correct the errors below.")
            # Debugging: Uncomment to see errors in console
...
def edit_invoice_view(request, invoice_id):
    ...
    js_data = {
        'products': {p.id: {'price': str(p.price)} for p in Product.objects.filter(is_active=True)},
        'batches': {p.id: [{'id': si.id, 'name': f"{si.batch_number or 'N/A'} (Avail: {si.quantity_available})", 'available': si.quantity_available} for si in p.stockitem_set.filter(quantity_available__gt=0)] for p in Product.objects.filter(is_active=True)},
        'services': {s.id: str(s.price) for s in Service.objects.filter(is_active=True)}
    }
    ...
    if request.method == 'POST':
        ...
        if invoice_form.is_valid() and item_formset.is_valid():
            with transaction.atomic():
                invoice_form.save() # Save the main invoice first
                
                # Django's formset.save() handles new, changed, and deleted forms automatically
                item_formset.save() 
                
                # Recalculate and save total amount after all items are processed
