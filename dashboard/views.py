...
def dashboard_view(request):
    ...
    # 1. Check for superuser status FIRST for unrestricted view
    ...
    # 2. If not superuser, check for a doctor profile
        ...
        appointments_base_qs = Appointment.objects.filter(doctor=doctor_profile)
        invoices_base_qs = Invoice.objects.filter(doctor=doctor_profile)
        lab_cases_base_qs = LabCase.objects.filter(doctor=doctor_profile)

    # 3. If not a doctor, it's a regular staff member, show all
    else:
        is_doctor = False
        appointments_base_qs = Appointment.objects.all()
        invoices_base_qs = Invoice.objects.all()
    ...
    # --- STATS CARDS LOGIC (This will now use the correct querysets) ---
